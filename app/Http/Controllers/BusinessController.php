    ...
    */
    ...
    public function getChangeBusiness()
    {
        $businessIds = User::select("business_id")
            ...
            ->get();
        $business = Business::whereIn("id", $businessIds->toArray())->pluck(
            "name",
            "id"
        );
        return view("business.partials.change_business_modal")->with(
            compact("business")
        );
    }
    ...
    public function changeBusiness(Request $request)
    {
        ...
        if (Hash::check($credentials["password"], $password)) {
            ...
            if ($verified) {
                ...
                if (Auth::check()) {
                    return response()->json([
                        "success" => true,
                        "msg" => __("business.success_change_business"),
                    ]);
                } else {
                    return response()->json([
                        "success" => false,
            ...
            return response()->json([
                "success" => false,
                "msg" => __("messages.wrong_password"),
            ]);
        }
    }
    /**
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            $validator = $request->validate(
                ...
                [
                    "name.required" => __("validation.required", [
                        "attribute" => __("business.business_name"),
                    ]),
                    "name.currency_id" => __("validation.required", [
                        "attribute" => __("business.currency"),
                    ]),
                    "country.required" => __("validation.required", [
                        "attribute" => __("business.country"),
                    ]),
                    "state.required" => __("validation.required", [
                        "attribute" => __("business.state"),
                    ]),
                    "city.required" => __("validation.required", [
                        "attribute" => __("business.city"),
                    ]),
                    "zip_code.required" => __("validation.required", [
                        "attribute" => __("business.zip_code"),
                    ]),
                    "landmark.required" => __("validation.required", [
                        "attribute" => __("business.landmark"),
                    ]),
                    "time_zone.required" => __("validation.required", [
                        "attribute" => __("business.time_zone"),
                    ]),
                    "email.email" => __("validation.email", [
                        "attribute" => __("business.email"),
                    ]),
                    "first_name.required" => __("validation.required", [
                        "attribute" => __("business.first_name"),
                    ]),
                    "username.required" => __("validation.required", [
                        "attribute" => __("business.username"),
                    ]),
                    ...
                    "fy_start_month.required" => __("validation.required", [
                        "attribute" => __("business.fy_start_month"),
                    ]),
                    "accounting_method.required" => __("validation.required", [
                        "attribute" => __("business.accounting_method"),
                    ]),
                ]
            );
            ...
            //upload logo
            $logo_name = $this->businessUtil->uploadFile(
                $request,
                "business_logo",
                "business_logos"
            );
            ...
            $this->businessUtil->newBusinessDefaultResources(
                $business->id,
                $user->id
            );
            $new_location = $this->businessUtil->addLocation(
                $business->id,
                $business_location
            );
            ...
            if (
                $is_installed_superadmin &&
                !empty($package_id) &&
                config("app.env") != "demo"
            ) {
                $package = \Modules\Superadmin\Entities\Package::find(
                    $package_id
                );
                if (!empty($package)) {
                    Auth::login($user);
            ...
            if(!Auth::guest()){
                return redirect("business")->with("status", $output);
            }
    ...
    public function getBusinessSettings()
    {
        ...
        $catalogue = Catalogue::select(
            "id",
            DB::raw("CONCAT(code, ' ', name) as full_name")
        )
            ->where("status", 1)
        ...
        if (empty($business->dashboard_settings)) {
            ...
            $dashboard_settings = json_decode(
                $business->dashboard_settings,
                true
            );
        ...
        if (empty($business->customer_settings)) {
            ...
            $customer_settings = json_decode(
                $business->customer_settings,
                true
            );
        }
        ...
        $product_taxes = $this->taxUtil
            ->getTaxGroups($business_id, "products")
            ->pluck("name", "id");
    ...
    public function getAccountingSettings()
    {
        ...
        if (!is_null($business->accounting_ordinary_expenses_id)) {
            array_push(
                $account_ids,
                $business->accounting_ordinary_expenses_id
            );
        }
        ...
        if (!is_null($business->accounting_vat_local_purchase_id)) {
            array_push(
                $account_ids,
                $business->accounting_vat_local_purchase_id
            );
        }
        ...
        $cost_main_account = Catalogue::select("code")
            ->where("id", $business->accounting_cost_id)
            ->first();
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            if (
                !empty(
                    $request->input("show_open_daily_z_cut_amount") &&
                        $request->input("show_open_daily_z_cut_amount") == 1
                )
            ) {
            ...
            if (
                !empty($request->input("enable_tooltip")) &&
                $request->input("enable_tooltip") == 1
            ) {
            ...
            $business_details[
                "show_expenses_on_sales_report"
            ] = $request->input("show_expenses_on_sales_report") ? 1 : 0;

            $business_details["enable_sell_delete"] = $request->input(
                "enable_sell_delete"
            )
                ? 1
                : 0;

            $business_details["enable_product_expiry"] =
                !empty($request->input("enable_product_expiry")) &&
                $request->input("enable_product_expiry") == 1
                    ? 1
                    : 0;
            ...
            $business_details["stock_expiry_alert_days"] = !empty(
                $request->input("stock_expiry_alert_days")
            )
                ? $request->input("stock_expiry_alert_days")
                : 30;
            ...
            if (
                !empty($request->input("purchase_in_diff_currency")) &&
                $request->input("purchase_in_diff_currency") == 1
            ) {
                $business_details["purchase_in_diff_currency"] = 1;
                $business_details["purchase_currency_id"] = $request->input(
                    "purchase_currency_id"
                );
                $business_details["p_exchange_rate"] = $request->input(
                    "p_exchange_rate"
                );
            } else {
            ...
            $business_details["dashboard_settings"] = json_encode(
                $dashboard_settings
            );
            ...
            $business_details["customer_settings"] = json_encode(
                $customer_settings
            );
            ...
            $business_details["product_settings"] = json_encode(
                $product_settings
            );
            ...
            $business_details["expense_settings"] = json_encode(
                $expense_settings
            );
            ...
            $financial_year = $this->businessUtil->getCurrentFinancialYear(
                $business->id
            );
    ...
    public function updateAnullSaleExpiry(Request $request)
    {
        ...
        try {
            ...
            $business_details["annull_sale_expiry"] =
                $request->annull_sale_expiry &&
                !empty($request->annull_sale_expiry)
                    ? 1
                    : 0;
            $business_id = request()
                ->session()
    ...
    { 
       ...
       if($request->ajax()){
        ...
        return DataTables::of($business)->toJson();
       }

       return view('business.index');
    }

    public function getCreateBusinessForm(){
    ...
    public function getEditBusinessForm($id){
        $business = Business::findOrFail($id);
        $location = BusinessLocation::where('business_id', $business->id)->orderBy('id', 'desc')->first();
    ...
    public function updateBusinessInformation(Request $request, $id){
        ...
        $location->update($business_location);
       
        return redirect("/business")->with("status", 200);
