...
<!-- Main Header -->
  <header class="main-header no-print">
    <a href="{{action([\App\Http\Controllers\HomeController::class, 'index'])}}" class="logo">
    ...
    <nav class="navbar navbar-static-top" role="navigation" style="height: 45px;">
      ...
      <div class="navbar-custom-menu">      
        ...
        @if($businessCount > 1)
        <a href="{{ action([\App\Http\Controllers\BusinessController::class, 'getChangeBusiness'])}}" title="{{ __('home.conected_business') }}" class="btn-flat pull-left m-8 hidden-sm btn-sm mt-10 btn_business_modal">
            <strong><i class="fa fa-briefcase"></i>&nbsp; @lang("business.change_business")</strong>
        </a>
        @endif

        @can('sell.create')
          <a href="{{action([\App\Http\Controllers\SellPosController::class, 'create'])}}" title="POS" data-toggle="tooltip" data-placement="bottom" class="btn btn-success btn-flat pull-left m-8 hidden-xs btn-sm mt-10">
